...
class DeviceService {
  ...
  static async getDeviceBenchmarkInfo() {
    // 检查缓存（设备信息基本不变，延长到 24 小时）
    if (this._cache.benchmarkInfo && (Date.now() - this._cache.lastUpdate) < 86400000) {
  ...
  /**
   * 获取详细 GPU 信息
   * @returns {Promise<Object>} GPU 信息对象
   */
  static async getDetailedGPUInfo() {
    // 检查缓存（GPU 信息不变，延长到 24 小时）
    if (this._cache.gpuInfo && (Date.now() - this._cache.lastUpdate) < 86400000) {
      ...
      if (wx.getBatteryInfo) {
        wx.getBatteryInfo({
          ...
          fail: (error) => {
            // 生产环境减少日志输出
            if (__DEV__) {
              logger.warn('异步获取电量信息失败，尝试同步 API:', error);
            }
            // 如果异步失败，尝试同步 API 作为备用
  ...
  static async getDeviceInfo() {
    ...
    return {
      ...systemInfo,
      model: friendlyModel,
      modelCode: systemInfo.model, // 保留原始型号代码
  ...
  static async _getSystemInfo() {
    try {
      ...
      // 生产环境减少日志输出
      if (__DEV__) {
        logger.warn('获取系统信息失败，使用备用方案:', error);
      }
      // 备用方案：返回基本系统信息
      return {
        platform: 'unknown',
    ...
    try {
      ...
      for (const service of ipServices) {
        try {
          ...
          // 生产环境减少日志输出
          if (__DEV__) {
            logger.warn(`IP 服务 ${service.url} 不可用:`, serviceError);
          }
          continue;
        }
      }
  ...
  static async getIPv4() {
    // 检查缓存（5分钟有效）
    if (this._ipCache.ipv4 && (Date.now() - this._ipCache.lastUpdate) < 300000) {
    ...
    try {
      ...
      for (const service of ipv4Services) {
        try {
          ...
          if (res.statusCode === 200 && res.data) {
            const ip = service.parser(res.data);
            // 验证是否为有效的 IPv4 地址：包含点号但不包含冒号
            if (ip && ip.length > 0 && ip.includes('.') && !ip.includes(':')) {
              // 生产环境减少日志输出
              if (__DEV__) {
                logger.log(`成功从 ${service.url} 获取 IPv4:`, ip);
              }
              // 更新缓存
              this._ipCache.ipv4 = ip;
              this._ipCache.lastUpdate = Date.now();
              return ip;
            } else {
              // 生产环境减少日志输出
              if (__DEV__) {
                logger.log(`${service.url} 返回的不是 IPv4 地址:`, ip);
              }
            }
          }
        } catch (serviceError) {
          // 生产环境减少日志输出
          if (__DEV__) {
            logger.warn(`IPv4 服务 ${service.url} 不可用:`, serviceError);
          }
      ...
      // 如果所有服务都失败，检查是否是域名未配置
      if (lastError && lastError.errMsg && lastError.errMsg.includes('url not in domain list')) {
        return '需配置域名';
      }
      // 生产环境减少日志输出
      if (__DEV__) {
        logger.warn('获取 IPv4 地址失败:', error);
      }
      return '获取失败';
    } catch (error) {
      // 生产环境减少日志输出
  ...
  static async getIPv6() {
    // 检查缓存（5分钟有效）
    if (this._ipCache.ipv6 && (Date.now() - this._ipCache.lastUpdate) < 300000) {
    ...
    try {
      ...
      for (const service of ipv6Services) {
        try {
          ...
          if (res.statusCode === 200 && res.data) {
            const ip = service.parser(res.data);
            // 验证是否为有效的 IPv6 地址：包含冒号但不包含点号
            if (ip && ip.length > 0 && ip.includes(':') && !ip.includes('.')) {
              // 生产环境减少日志输出
              if (__DEV__) {
                logger.log(`成功从 ${service.url} 获取 IPv6:`, ip);
              }
              // 更新缓存
              this._ipCache.ipv6 = ip;
              this._ipCache.lastUpdate = Date.now();
              return ip;
            } else {
              // 生产环境减少日志输出
              if (__DEV__) {
                logger.log(`${service.url} 返回的不是 IPv6 地址:`, ip);
              }
            }
          }
        } catch (serviceError) {
          // 生产环境减少日志输出
          if (__DEV__) {
            logger.warn(`IPv6 服务 ${service.url} 不可用:`, serviceError);
          }
          lastError = serviceError;
          continue;
        }
      ...
      // 生产环境减少日志输出
      if (__DEV__) {
        logger.warn('获取 IPv6 地址失败:', error);
      }
      return '不支持';
    }
  }
